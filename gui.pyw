...
class DealApp(ctk.CTk):
    ...
    def generate_kp_verkh(self):
        if deal_id := self.get_deal_id():
            self.safe_call(self.main_module.generate_kp_verkh, deal_id, f"КП для верха для {deal_id} создано.")
    def generate_kp_lsho(self):
        if deal_id := self.get_deal_id():
            self.safe_call(self.main_module.generate_kp_lsho, deal_id, f"КП для ЛШО для {deal_id} создано.")
    def refresh_data(self):
        if deal_id := self.get_deal_id():
            self.safe_call(self.main_module.refresh_data, deal_id, f"Данные для {deal_id} обновлены.")
    ...
    def generate_kp_lsho (self):
        if deal_id := self.get_deal_id():
            self.safe_call(self.main_module.generate_kp_lsho, deal_id, f"КП ЛШО для {deal_id} создано.")

    def generate_kp_verch(self):
        if deal_id := self.get_deal_id():
