...
def fill_excel(products, output_path):
    ...
    for i, p in enumerate(products):
        ...
        ws_calc[f"E{r}"] = p["price_purchase"]
...
def import_data (deal_id):
    ...
    for row in rows:
        ...
        if weight_g == 0 or weight_g == None:
            weight_g = product.get("property214", {}).get("value", 0)
            print(weight_g)
        length_mm = product.get("length", 0)
        width_mm = product.get("width", 0)
        height_mm = product.get("height", 0)
        supplier = product.get("property228", {}).get("value", 0)
        vat_in = product.get("property236", {}).get("value", 0)
        link = product.get("property206", {}).get("value", "").strip()
        price_purchase = product.get("property216", {}).get("value", 0)
        volume_m3 = product.get("property214", {}).get("value", 0)
