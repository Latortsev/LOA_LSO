...
class StdoutRedirector:
    def __init__(self, text_widget):
        ...
        self.original_stdout = sys.__stdout__ or sys.stdout  # безопасно

    def write(self, message):
        if not message.strip():
            return

        # Печатаем в консоль, если можно
        try:
...
# === Устанавливаем до импорта main ===
stdout_redirector_instance = None

def set_stdout_redirector(text_widget):
    global stdout_redirector_instance
    stdout_redirector_instance = StdoutRedirector(text_widget)
    sys.stdout = stdout_redirector_instance
